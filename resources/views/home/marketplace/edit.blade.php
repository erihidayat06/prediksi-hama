...
@section('content')
                        ...
                        <form action="{{ route('produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <div id="preview-container" class="d-flex flex-wrap gap-2">
                                    ...
                                    @endphp
                                    @foreach ($gambar as $index => $src)
                                        <div class="position-relative" data-gambar-lama="{{ $src }}">
                                            <img src="{{ asset('storage/' . $src) }}" class="img-preview">
                                            <button type="button" class="remove-image"
                                                onclick="hapusGambarLama(this)">×</button>
                                        </div>
                                    @endforeach
                            ...
                            <div class="mb-3">
                                ...
                                <input class="form-control @error('judul') is-invalid @enderror" type="text"
                                    name="judul" id="judul" value="{{ old('judul', $produk->judul) }}">
                                @error('judul')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input class="form-control @error('no_tlp') is-invalid @enderror" type="text"
                                    name="no_tlp" id="no_tlp" value="{{ old('no_tlp', $produk->no_tlp) }}">
                                @error('no_tlp')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="satuan" id="satuan"
                                    ...
                                    <option value="kg" {{ old('satuan', $produk->satuan) == 'kg' ? 'selected' : '' }}>
                                        Kilogram (kg)</option>
                                    <option value="g" {{ old('satuan', $produk->satuan) == 'g' ? 'selected' : '' }}>
                                        Gram (g)</option>
                                    <option value="ltr" {{ old('satuan', $produk->satuan) == 'ltr' ? 'selected' : '' }}>
                                        Liter (ltr)</option>
                                    <option value="pcs" {{ old('satuan', $produk->satuan) == 'pcs' ? 'selected' : '' }}>
                                        Pcs</option>
                                    <option value="ikat"
                                        {{ old('satuan', $produk->satuan) == 'ikat' ? 'selected' : '' }}>Ikat</option>
                            ...
                            <div class="mb-3">
                                ...
                                <input class="form-control @error('kecamatan') is-invalid @enderror" type="text"
                                    name="kecamatan" id="kecamatan" value="{{ old('kecamatan', $produk->kecamatan) }}">
                                @error('kecamatan')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input class="form-control @error('desa') is-invalid @enderror" type="text"
                                    name="desa" id="desa" value="{{ old('desa', $produk->desa) }}">
                                @error('desa')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('alamat') is-invalid @enderror" name="alamat" id="alamat" rows="3">{{ old('alamat', $produk->alamat) }}</textarea>
                                @error('alamat')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi" id="deskripsi" rows="4">{{ old('deskripsi', $produk->deskripsi) }}</textarea>
                                @error('deskripsi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="status" id="status"
                                    ...
                                    <option value="1" {{ old('status', $produk->status) == true ? 'selected' : '' }}>
                                        Aktif</option>
                                    <option value="0"
                                        {{ old('status', $produk->status) == false ? 'selected' : '' }}>Nonaktif</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
    ...
    <script>
        ...
        function hapusGambarLama(button) {
            ...
            const gambarPath = container.getAttribute('data-gambar-lama');

            // Tambahkan input hidden agar dikirim ke server
            const input = document.createElement('input');
            input.type = 'hidden';
            input.name = 'hapus_gambar[]';
            input.value = gambarPath;
