        ...
        <div class="card">
            <div class="card-body">
                <div class="card-title">
                    <h5 class="fw-bold">Edit Kegiatan Tanaman</h5>
                </div>
                <form action="{{ route('gap.update', ['tanaman' => $tanaman->nm_tanaman, 'gap' => $gap->id]) }}"
                    ...
                    <div class="mb-3">
                        ...
                        @enderror
                        <img id="preview" src="{{ asset($gap->gambar) }}" class="img-fluid mt-2" width="150">
                    </div>
                    <div class="mb-3">
                        <label for="usia" class="form-label">Usia</label>
                        <input type="text" class="form-control @error('usia') is-invalid @enderror" id="usia"
                            name="usia" value="{{ old('usia', $gap->usia) }}">
                        @error('usia')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('kegiatan') is-invalid @enderror" id="kegiatan"
                            name="kegiatan" value="{{ old('kegiatan', $gap->kegiatan) }}">
                        @error('kegiatan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
